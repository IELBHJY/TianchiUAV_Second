...
public class Main {
	...
	// write your code here
        ...
        readCity();
        AStar aStar=new AStar(6);
        long start = System.currentTimeMillis();
        for (int i = 0; i <10; i++) {
            Point endPoint;
            List<Point> set =new ArrayList<>();
            aStar.move(new Point(141,327,i*5), xCity[i]-1, yCity[i]-1,barrier);
        ...
        System.out.println("求解用时"+(end - start)/1000);
    }
    ...
    private static void test(Queue<Integer> queues){
        int[] q=new int[queues.size()];
        int i=0;
